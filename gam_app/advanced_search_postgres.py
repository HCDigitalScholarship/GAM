...
def make_queryset(search_string, field):  # this will return queryset in SET
    query = SearchQuery(search_string, config='spanish')
    if field == "Cualquier Campo":
        # return Persona, Lugar, and Imagen objects.	
        vector = SearchVector('nombre_de_la_persona', 'género', 'etnicidad', config='spanish')
      
        q1 = set(
            Persona.objects.annotate(search=vector, rank=SearchRank(vector, query)).filter(search=query).order_by("-rank"))
        vector2 = SearchVector('nombre_del_lugar', config='spanish')
        q2 = set(Lugar.objects.annotate(search=vector2, rank=SearchRank(vector2, query)).filter(search=query).order_by("-rank"))
        # it is still a question whether there is a need to return Imagen objects
        vector3 = SearchVector('texto_de_OCR', 'notas', config='spanish')
	
        q3 = set(
            Imagen.objects.annotate(search=vector3, rank=SearchRank(vector3, query)).filter(search=query).order_by("-rank"))
        queryset = q1 | q2 | q3
        # print('queryset', queryset)
    elif field == 'Persona':
        vector = SearchVector('nombre_de_la_persona', config='spanish')
        q1 = set(Persona.objects.annotate(search=vector, rank=SearchRank(vector, query)).filter(search=query).order_by("-rank"))
        ...
        vector = SearchVector('nombre_del_lugar', config='spanish')
	
        q1 = set(Lugar.objects.annotate(search=vector, rank=SearchRank(vector, query)).filter(search=query).order_by("-rank"))
        # q2 = set(Imagen.objects.filter(Q(texto_de_OCR__icontains=search_string) | Q(traducción__icontains=search_string)))
        queryset = q1  # | q2
        # print('queryset', queryset)
    elif field == 'Género':
        vector = SearchVector('género', config='spanish')
        queryset = set(Persona.objects.annotate(search=vector, rank=SearchRank(vector, query)).filter(search=query).order_by("-rank"))
        # print('queryset', queryset)
    elif field == 'Etnicidad':
        vector = SearchVector('etnicidad', config='spanish')
        queryset = set(Persona.objects.annotate(search=vector, rank=SearchRank(vector, query)).filter(search=query).order_by("-rank"))
        # print('queryset', queryset)
    else:
