...
def filtrar_imagenes(request):
    ...
    photo_dict = {}
    for photo in photos:
        name = photo.file.split('-')[-1].split('.')[0].split('_')
        caja, legajo, carpeta = name[2], name[3], name[4]
        caso = Caso.objects.get(carpetas__caja=caja, carpetas__legajo=legajo, carpetas__carpeta=carpeta)

        try:
            photo_dict[caso].append(photo)
        except:
            photo_dict[caso] = []
            photo_dict[caso].append(photo)
    ...
    #sort the photo_list
    true_photo_list = []
    for caso in photo_dict:
        for photo in photo_dict[caso]:
            true_photo_list.append((caso.nombre_del_caso,photo))

    print(true_photo_list)
    context = {'photo_list': true_photo_list, 'filter_list': filter_list, 'photo_mostrar': photo_mostrar, }
